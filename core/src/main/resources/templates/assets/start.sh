...
readonly JAVA17_JVM_OPTS="-XX:-UseContainerSupport"
readonly JAVA21_JVM_OPTS="-XX:+UseZGC -XX:+ZGenerational -Dspring.threads.virtual.enabled=true"
...
选项:
  ...
  --skywalking-opts <opts>     追加 SkyWalking/JVM 代理选项
...
环境变量:
  ...
  SKYWALKING_AGENT_JAR         显式指定 skywalking-agent.jar 路径，优先级最高
  SKYWALKING_AGENT_HOME        Agent 下载/缓存根目录，默认 $HOME/.skywalking/agents
  SKYWALKING_AGENT_DOWNLOAD_DIR Agent tgz 下载缓存目录，默认 $HOME/Downloads
  SKYWALKING_AGENT_VERSION     Agent 版本，默认读取 pom.xml 的 skywalking.version；可设为 latest 从 Apache 发布目录检测
  SKYWALKING_AGENT_AUTO_DOWNLOAD 是否自动下载缺失 Agent，默认 true；设为 false 可禁用下载
  SKYWALKING_AGENT_DOWNLOAD_URL 自定义 Agent tgz 下载地址
  SKYWALKING_AGENT_ADL         自定义 adl 下载脚本路径；未设置时自动检测 PATH
  SKYWALKING_AGENT_SEARCH_DIRS  额外 Agent jar 查找目录，多个目录用冒号分隔；默认只查缓存根目录
  SKYWALKING_AGENT_LOG_LEVEL   Agent 自身日志级别，默认 WARN
  SKYWALKING_AGENT_LOG_OUTPUT  Agent 自身日志输出，默认 FILE
  SKYWALKING_APP_AGENT_DIR     当前应用独立 Agent 目录，默认 $HOME/.skywalking/agents/apps/$APP_NAME
    ...
    if [ -f "$PID_FILE" ]; then
        ...
        if kill -0 "$pid" 2>/dev/null; then
            if [ "$force_restart" = true ]; then
                log_info "应用 $APP_NAME 已在运行 (PID: $pid)，使用 -f 参数将强制停止后重启..."
                # 调用 stop.sh -f 来停止应用
...
load_skywalking_agent_script() {
    local skywalking_script="${CURRENT_DIR}/skywalking-agent.sh"

    if [ -f "$skywalking_script" ]; then
        # shellcheck source=/dev/null
        source "$skywalking_script"
    else
        log_warn "SkyWalking Agent 脚本不存在，跳过自动启用: $skywalking_script"
    fi
}
...
build_jvm_opts() {
    ...
    # 检测Java版本，如果是JDK 17，则添加相应的JVM参数避免Cgroup错误
    local java_version
    if command -v java >/dev/null 2>&1; then
        ...
        if [ "$major_version" = "17" ]; then
            # 添加禁用注册MBean的参数，避免Druid的Cgroup相关NPE
            jvm_opts="$jvm_opts $JAVA17_JVM_OPTS"
        elif [ "$major_version" = "21" ]; then
            # Java 21 最佳实践: 启用分代ZGC和虚拟线程
            jvm_opts="$jvm_opts $JAVA21_JVM_OPTS"
        fi
    fi
    ...
    if [ "${DEBUG_MODE:-}" = "debug" ]; then
        ...
        local debug_address="${DEBUG_ADDRESS:-*:$debug_port}"
        local debug_opts="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=$debug_address"
        jvm_opts="$jvm_opts $debug_opts"
    fi

    if command -v configure_skywalking_jvm_opts >/dev/null 2>&1; then
        jvm_opts=$(configure_skywalking_jvm_opts "$jvm_opts")
    fi

    # 添加SkyWalking代理选项（来自命令行参数）
    if [ -n "${SKYWALKING_AGENT_OPTS:-}" ]; then
...
get_process_port() {
    ...
    # 如果ss命令不可用，优先尝试lsof（跨平台兼容性更好，尤其是macOS）
    if [ -z "$ports" ] && command -v lsof >/dev/null 2>&1; then
        ports=$(lsof -a -p "$pid" -iTCP -sTCP:LISTEN -P -n 2>/dev/null | awk 'NR>1 {print $9}' | sed 's/.*://' | sort -u | tr '\n' ' ' | sed 's/ $//')
    fi

    # 如果都没有，则回退到netstat (注意: -tlpn在macOS上会报错)
    if [ -z "$ports" ] && command -v netstat >/dev/null 2>&1; then
        ports=$(netstat -anp 2>/dev/null | grep "$pid/java" | awk '{print $4}' | sed 's/.*://' | sort -u | tr '\n' ' ' | sed 's/ $//' || echo "")
    fi
...
check_startup_status() {
    ...
    if date --version >/dev/null 2>&1; then
        ...
        start_ts=$(date -u +"%Y-%m-%dT%H:%M:%S%z" | sed 's/\(..\)$/:\1/')
    fi
    ...
    if stat --version >/dev/null 2>&1; then
        ...
        log_start_size=$(stat -f%z "$LOG_FILE")
    fi
...
batch_start_all_services() {
    ...
    for service_name in "${SERVICE_DEPENDENCY_ORDER[@]}"; do
        ...
        # 匹配类似 "java -jar /path/to/app-service/lib/app-service-1.0.jar" 的进程
...
main() {
	...
	if [[ -z ${SERVICE_DEPENDENCY_ORDER+x} ]]; then
	    ...
	    if [ -f "${BASE_DIR}/.service-order.conf" ]; then
	        ...
	        log_warn "将按 ${SERVICES_BASE_DIR} 下的服务目录名称排序启动。请运行 package.sh 并重新部署以生成依赖顺序文件。"
	        SERVICE_DEPENDENCY_ORDER=()
	        if [ -d "$SERVICES_BASE_DIR" ]; then
	            while IFS= read -r service_dir; do
	                SERVICE_DEPENDENCY_ORDER+=("$(basename "$service_dir")")
	            done < <(find "$SERVICES_BASE_DIR" -maxdepth 1 -mindepth 1 -type d | sort)
	        fi
	    fi
	fi
    # --- 1. 参数解析 ---
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -n|--no-startup-log) no_startup_log=true; shift 1 ;;
    ...
    start_application "$jar_file" "$final_jvm_opts" || die "启动应用失败"
