...
<body class="no-skin">
	...
	<form action="/${table.beanName!}/list" method="post">
		<fieldset>
			<#list table.fields as field><#if field.comment?has_content && !field.isPrimarykey()>
			...
			</#if> </#list>  <input type="submit" value="搜索" style="margin-left: 90%" class="col-md-1 btn btn-primary btn-sm" />
		</fieldset>
	</form>
