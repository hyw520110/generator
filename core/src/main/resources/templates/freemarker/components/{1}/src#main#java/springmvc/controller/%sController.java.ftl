...
public class ${className!} <#if superControllerClass??>extends ${superControllerClass!}<${entityName!}Service,${entityName!}></#if> {
<#if "plus"!=mapperType>
<#assign sName = StringUtils.lowercaseFirst(serviceName)!>
<#assign eName = StringUtils.capitalFirst(entityName)!>
...
<#if table.primarykeyFields?size gt 0>
    ...
    public <#if THYMELEAF??>${eName}<#else> ModelAndView </#if> getInfo(<#list table.primarykeyFields as field>@PathVariable(value = "${field.propertyName}") final ${field.fieldType.type} ${field.propertyName} <#if field?has_next>,</#if> </#list>,@PathVariable(value = "method")String method){
        ${eName} bean= ${sName!}.getById(<#list table.primarykeyFields as field>${field.propertyName}<#if field?has_next>,</#if></#list>);
...
<#if !THYMELEAF??>
    ...
    public <#if THYMELEAF??>Result<?><#else> ModelAndView </#if> save(HttpServletRequest req,@Validated @ModelAttribute("bean") ${eName} bean,BindingResult result){
<#if THYMELEAF??>
...
<#else>
    ...
    public <#if THYMELEAF??>Result<?><#else> ModelAndView </#if> update(@Valid ${eName} bean){
<#if THYMELEAF??>		
