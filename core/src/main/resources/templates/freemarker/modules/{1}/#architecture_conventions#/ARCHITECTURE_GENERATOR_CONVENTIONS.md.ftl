...
- `@DubboReference` 必须引用统一默认值，不在注解中直接写超时、重试和降级字符串。
...
- 高并发 I/O 服务默认开启虚拟线程；确需关闭时必须说明原因。

## 已生成的通用能力

| 能力 | 说明 |
|------|------|
| 统一响应与分页 | 复用 `Result`、`PageResult`、状态码等基础返回结构 |
| 基础实体与持久化 | 复用 `BaseEntity`、MyBatis-Plus 配置、自动填充和 Mapper XML 模板 |
| 异常与上下文 | 复用全局异常处理、请求上下文拦截和租户上下文能力 |
| Dubbo 默认值 | 生成 `RpcReferenceDefaults`，统一消费端超时、重试和降级口径 |
| 缓存与消息 | 按 Feature 生成 Redis、RocketMQ 等基础组件骨架 |
| 运维脚本 | 可按需复制通用打包、启停、状态查看、部署脚本和容器化模板 |
...
| 配置 | 确认 `application-common.yml` 中的端口、注册中心、线程、Mapper 路径和上下文头校验已按环境调整 |
