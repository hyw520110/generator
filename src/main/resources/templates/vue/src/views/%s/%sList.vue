...
#foreach($field in ${table.fields})
    ...
    <create-form ref="createModal" @ok="handleOk" @add="handleAddOk" @edit="handleEditOk" />
  </a-card>
</template>

<script>
import { STable, Ellipsis } from '@/components'
import CreateForm from './${table.beanName}Create'
import { getList, add, edit#if(${table.hasPrimarykeys}), del#end } from '@/api/${table.beanName}'
...
#foreach($field in ${table.fields})
  ...
  methods: {
    ...
    handleDel (record) {
      if (record.status !== 0) {
#set($info='this.$message.info(`${record.no} 删除成功`)')
#set($error='this.$message.error(`${record.no} 删除失败`)')
        ${info}
      } else {
        ${error}
      }
#if(${table.hasPrimarykeys})
      del(record.postId).then(res => {
